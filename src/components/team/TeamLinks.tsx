...
function getUrlIcon(url: string) {
  const u = url.toLowerCase();
  if (u.includes("drive.google.com")) return { icon: "G", color: "text-blue-600 bg-blue-50", label: "Google Drive" };
  if (u.includes("docs.google.com")) return { icon: "G", color: "text-blue-600 bg-blue-50", label: "Google Docs" };
  if (u.includes("sheets.google.com")) return { icon: "G", color: "text-green-600 bg-green-50", label: "Google Sheets" };
  if (u.includes("meet.google.com")) return { icon: "M", color: "text-green-600 bg-green-50", label: "Google Meet" };
  if (u.includes("youtube.com") || u.includes("youtu.be")) return { icon: "Y", color: "text-red-600 bg-red-50", label: "YouTube" };
  if (u.includes("github.com")) return { icon: "GH", color: "text-slate-800 bg-slate-100", label: "GitHub" };
  if (u.includes("figma.com")) return { icon: "F", color: "text-purple-600 bg-purple-50", label: "Figma" };
  if (u.includes("notion.so") || u.includes("notion.site")) return { icon: "N", color: "text-slate-800 bg-slate-100", label: "Notion" };
  if (u.includes("trello.com")) return { icon: "T", color: "text-blue-600 bg-blue-50", label: "Trello" };
  if (u.includes("slack.com")) return { icon: "S", color: "text-purple-600 bg-purple-50", label: "Slack" };
  if (u.includes("discord.com") || u.includes("discord.gg")) return { icon: "D", color: "text-indigo-600 bg-indigo-50", label: "Discord" };
  return null;
}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-slate-100 rounded-lg p-1 overflow-x-auto">
          <button
            ...
            className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap", filter === "all" ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700")}
          >
            All ({links.length})
          </button>
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap", filter === cat.value ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700")}
              >
                {cat.label} ({count})
              </button>
            );
      ...
      {filteredLinks.length === 0 ? (
        <div className="text-center py-12 bg-white border border-slate-200 rounded-2xl">
          ...
          <p className="text-sm text-slate-500 mb-3">No links yet</p>
          ...
          {filteredLinks.map((link) => {
            ...
            return (
              <a
                ...
                className="bg-white border border-slate-200 rounded-xl p-4 hover:border-indigo-200 hover:shadow-sm transition-all group block"
              >
                <div className="flex items-start gap-3">
                  <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center text-xs font-bold shrink-0", urlInfo?.color || "text-slate-600 bg-slate-100")}>
